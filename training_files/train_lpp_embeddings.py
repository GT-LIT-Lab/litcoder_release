...
class LPPTrainer:
    ...
    def setup_logger(self):
        ...
        run_name = f"lpp-story{self.config.get('story_idx', 'NA')}-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
        if backend == "wandb":
            try:
    ...
    def prepare_data(self) -> Dict[str, np.ndarray]:
        ...
        words = self.assembly.get_words()[story_idx]
    ...
    def train(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Log the saved path when using wandb
