...
from encoding.features import WordRateFeatureExtractor
...
class LPPTrainer:
    ...
    def setup_logger(self):
        ...
        run_name = f"lpp-wordrate-story{self.config.get('story_idx', 'NA')}-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
        if backend == "wandb":
            try:
    ...
    def prepare_data(self) -> Dict[str, np.ndarray]:
        ...
        word_rates = self.assembly.get_word_rates()[story_idx]
        ...
        delayed_features = FIR.make_delayed(features, delays)
    ...
    def train(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Log path table only when using wandb
