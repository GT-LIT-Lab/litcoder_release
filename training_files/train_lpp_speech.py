...
class LPPTrainer:
    ...
    def setup_logger(self):
        ...
        run_name = f"lpp-speech-story{self.config.get('story_idx', 'NA')}-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
        if backend == "wandb":
            try:
    ...
    def prepare_data(self) -> Dict[str, np.ndarray]:
        ...
        # all subjects have the same wav file
        wav_path = f"/storage/coda1/p-aivanova7/0/shared/taha/SAE_encoding_models/lit_encoding/data/little_prince/neural_data/sub-EN073/task-lppEN_section-{story_idx+1}.wav"  # replace with your own path
        print(f"this is the wav path: {wav_path}")
