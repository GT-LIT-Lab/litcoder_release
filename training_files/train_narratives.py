...
class NarrativesTrainer:
    ...
    def prepare_data(self) -> Dict[str, np.ndarray]:
        ...
        for story in self.assembly.stories:
            ...
            print(f"this is the tr times: {tr_times}")
    ...
    def log_metrics(self, metrics: Dict[str, Union[float, List[float]]]):
        ...
        )  # you have to add your own paths here. We provide the masks under masks.
        a1_mask = np.load(
...
def parse_args():
    ...
    # Weights & Biases parameters (required if using wandb)
    parser.add_argument(
        "--wandb_project_name",
        type=str,
        required=False,
