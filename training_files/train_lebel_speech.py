...
from encoding.utils import zs, ActivationCache, ModelSaver, SpeechActivationCache
...
class LebelTrainer:
    ...
    def prepare_data(self) -> Dict[str, np.ndarray]:
        ...
        for story in train_stories + test_stories:
            ...
            wav_path = f"/storage/coda1/p-aivanova7/0/shared/taha/SAE_encoding_models/lit_encoding/data/lebel/audio_files/{story}.wav"  # replace with your own path
            cache_key = self.speech_cache.get_cache_key(
                audio_id=wav_path,  # or a content hash if paths can move
            ...
            lazy_cache = self.speech_cache.load_multi_layer_activations(cache_key)

            if lazy_cache is not None:
                self.logger.info(f"Loaded cached speech activations for story {story}")
                # sanity-check the important knobs match
                try:
                    lazy_cache.validate_params(
                        expected={
                            "model_name": self.embeddings_model.model_name,
                            "chunk_size": self.embeddings_model.chunk_size,
                            "context_size": self.embeddings_model.context_size,
                    ...
                    self.logger.warning(f"Parameter mismatch for story {story}: {e}")
                    lazy_cache = None

            if lazy_cache is not None:
                # Load the specific layer we need
                features = lazy_cache.get_layer(
                    self.config["layer_idx"]
                )  # [n_chunks, D]
                times = lazy_cache.get_times()  # [n_chunks]
                if times is None:
                    raise RuntimeError(
                        "Cached speech activations missing 'times' array."
                    )
            else:
                # Not cached → compute all layers once, then cache
                self.logger.info(f"Computing speech activations for story {story}")
                layer_to_feats, times = self.embeddings_model.extract_all_layers(
                    wav_path
                )
